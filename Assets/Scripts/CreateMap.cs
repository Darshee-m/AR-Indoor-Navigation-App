    ...
    void Update() {
        if (mFrameUpdated) {
            ...
            if (shouldRecordWaypoints) {
                ...
                pos.y = -.5f;
                shapeManager.AddShape(pos, Quaternion.Euler(Vector3.zero), false, false);
...
/*    public void labeltoMeta()
    {
        Transform player = Camera.main.transform;
        Vector3 pos = player.position;
        Debug.Log(player.position);
        pos.y = -.5f;
        shapeManager.AddShape(pos, Quaternion.Euler(Vector3.zero), false, true);
        ...
        if (shouldSaveMap) {
            ...
            LibPlacenote.Instance.SaveMap(
                (mapId) => {
                    ...
                    //JObject ini_loc = GetComponent<Inilocation>().Loc2JSON();

                    //userdata["ini_loc"] = ini_loc;
